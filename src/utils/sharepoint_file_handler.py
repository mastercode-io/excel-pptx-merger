...
class SharePointFileHandler:
    ...
    def _load_credentials(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"🔑   GRAPH_CLIENT_ID: {'SET (' + env_client_id[:8] + '...)' if env_client_id else 'NOT SET'}"
            )
            logger.info(
                f"🔑   GRAPH_CLIENT_SECRET: {'SET (' + env_client_secret[:4] + '...)' if env_client_secret else 'NOT SET'}"
            )
            logger.info(
                f"🔑   GRAPH_TENANT_ID: {'SET (' + env_tenant_id[:8] + '...)' if env_tenant_id else 'NOT SET'}"
            )
            ...
            logger.info(
                f"🔧 Calling get_graph_api_credentials(tenant_id='{config_tenant_id}')..."
            )
            self.graph_credentials = get_graph_api_credentials(config_tenant_id)
            logger.info(
                f"🔧 get_graph_api_credentials() returned: {type(self.graph_credentials)} - {bool(self.graph_credentials)}"
            )

            if self.graph_credentials:
                logger.info(
                    "✅ Graph API credentials loaded successfully - SharePoint access enabled"
                )
                logger.info(
                    f"✅ Credential keys available: {list(self.graph_credentials.keys())}"
                )
                logger.debug(
                ...
                logger.error(
                    "❌ Graph API credentials are None - SharePoint access disabled"
                )
                logger.error("❌ This will cause SharePoint file access to fail!")
                logger.error(
                    "❌ Check environment variables: GRAPH_CLIENT_ID, GRAPH_CLIENT_SECRET, GRAPH_TENANT_ID"
                )
                logger.error(
                    "❌ Or ensure config/graph_api.env file exists and is readable"
                )
    ...
    def is_configured(self) -> bool:
        ...
        logger.debug(
            f"SharePoint configuration check - enabled: {sharepoint_enabled}, credentials: {credentials_available}"
        )
    ...
    def get_credentials_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "client_id_present": bool(
                self.graph_credentials and self.graph_credentials.get("client_id")
            ),
            "client_secret_present": bool(
                self.graph_credentials and self.graph_credentials.get("client_secret")
            ),
            "tenant_id_present": bool(
                self.graph_credentials and self.graph_credentials.get("tenant_id")
            ),
        }
