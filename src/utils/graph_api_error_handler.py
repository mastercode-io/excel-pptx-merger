...
class GraphAPIErrorHandler:
    ...
    def get_retry_delay(self, attempt: int, error: Exception = None) -> float:
        ...
        if (error and hasattr(error, 'response') and 
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        ...
                        raise ExcelProcessingError(f"Graph API operation failed after {max_retries + 1} attempts: {e}")
                    ...
                    if not handler.is_retryable_error(e):
                        ...
                        raise ExcelProcessingError(f"Graph API operation failed with non-retryable error: {e}")
...
def safe_graph_operation(operation_name: str, error_handler: Optional[GraphAPIErrorHandler] = None):
    ...
    class GraphOperationContext:
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            ...
            if exc_type is None:
                logger.debug(f"Graph API operation completed: {self.name} ({duration:.2f}s)")
            else:
                self.handler.handle_error(exc_val, self.name, {
...
def validate_graph_response(response: requests.Response, operation: str) -> None:
    ...
    try:
        ...
        if response.status_code in GraphAPIErrorHandler.RETRYABLE_STATUS_CODES:
            raise GraphAPIRetryableError(f"{operation} failed with retryable error: {e}")
        else:
