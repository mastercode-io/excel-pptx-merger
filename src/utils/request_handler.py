...
class RequestPayloadDetector:
    ...
    def log_request_info(request: Request) -> None:
        ...
        logger.info("📊 User-Agent: %s", request.headers.get('User-Agent', 'Not specified'))
        logger.info("📊 Content-Type: %s", request.headers.get('Content-Type', 'Not specified'))
        logger.info("📊 Content-Length: %s", request.headers.get('Content-Length', 'Not specified'))
...
class PayloadParser:
    ...
    def get_file(self, field_name: str, required: bool = True) -> Optional[Union[FileStorage, io.BytesIO]]:
        ...
        if self.is_json_request:
            ...
            if not file_b64:
                if required:
                    raise ValidationError(f"{field_name} (base64) is required in JSON mode")
                return None
            ...
            try:
                ...
                file_obj.filename = json_data.get(f"{field_name}_name", f"{field_name}.bin")
    ...
    def get_param(self, param_name: str, default: Any = None, required: bool = False) -> Any:
    ...
    def get_json_param(self, param_name: str, default: Any = None, required: bool = False) -> Any:
