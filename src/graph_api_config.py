...
class GraphAPIConfig:
    ...
    def _load_config(self) -> None:
        ...
        logger.info(
            "🔧 Loading from environment variables (takes precedence over file)..."
        )
        self._load_from_env()
    ...
    def _load_from_env(self) -> None:
        ...
        logger.info(
            f"🔑 GRAPH_CLIENT_ID: {'SET (' + client_id[:8] + '...)' if client_id else 'NOT SET'}"
        )
        logger.info(
            f"🔑 GRAPH_CLIENT_SECRET: {'SET (' + client_secret[:4] + '...)' if client_secret else 'NOT SET'}"
        )
        logger.info(
            f"🔑 GRAPH_TENANT_ID: {'SET (' + tenant_id[:8] + '...)' if tenant_id else 'NOT SET'}"
        )
        ...
        logger.info(
            f"🔧 Credentials loaded from environment: {len([k for k, v in self._credentials.items() if v])} of 3 set"
        )
    ...
    def is_configured(self) -> bool:
        ...
        if not is_configured:
            logger.error(
                "❌ GraphAPIConfig is not configured - missing required credentials"
            )
            if not client_id:
...
def get_graph_api_credentials(
    ...
    try:
        ...
        if is_configured:
            ...
            logger.info(
                f"✅ Credential keys: {list(credentials.keys()) if credentials else 'None'}"
            )
            ...
            if tenant_id:
                ...
                logger.info(
                    f"🔄 Overrode tenant_id: '{original_tenant}' -> '{tenant_id}'"
                )
            ...
            logger.error(
                "❌ This means environment variables or config file are missing"
            )
