...
class ExcelRangeExporter:
    ...
    def export_ranges(self, workbook_path: str, range_configs: List[RangeImageConfig]) -> List[RangeImageResult]:
    ...
    def _export_single_range(self, item_id: str, config: RangeImageConfig, 
                           available_worksheets: List[str]) -> RangeImageResult:
        ...
        try:
            ...
            if not self.graph_client.validate_range(item_id, config.sheet_name, config.range):
            ...
            logger.info(f"Rendering range {config.range} from sheet '{config.sheet_name}' as image")
    ...
    def _save_image_to_temp_file(self, image_data: bytes, field_name: str, 
        ...
        temp_path = self.temp_manager.create_temp_file(suffix=f".{output_format.lower()}")
    ...
    def validate_config(self, config: RangeImageConfig) -> bool:
        ...
        try:
            ...
            if config.dpi < 72 or config.dpi > 600:
                raise ValidationError(f"DPI must be between 72 and 600, got: {config.dpi}")
...
def create_range_configs_from_dict(range_configs_data: List[Dict[str, Any]]) -> List[RangeImageConfig]:
