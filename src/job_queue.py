...
class JobQueue:
    ...
    def __init__(self):
        ...
        self._allowed_endpoints = {"/api/v1/extract", "/api/v1/merge", "/api/v1/update"}
    ...
    def create_job(
        self, endpoint: str, payload: Dict[str, Any], client_ip: str = None
    ) -> str:
        """
        Create a new job
        ...
        with self._lock:
            ...
            if not self.validate_endpoint(endpoint):
                raise ValueError(
                    f"Endpoint '{endpoint}' is not allowed for async processing"
                )
            ...
            if client_ip:
                ...
                if active_jobs >= self._max_jobs_per_client:
                    raise ValueError(
                        f"Too many active jobs. Maximum {self._max_jobs_per_client} allowed."
                    )

            # Create job
            job_id = self.generate_job_id()
            job = Job(
                id=job_id, endpoint=endpoint, payload=payload, status=JobStatus.PENDING
            )
    ...
    def get_job_result(
        self, job_id: str, cleanup: bool = True
        ...
        with self._lock:
            ...
            for job_id, job in self._jobs.items():
                ...
                if age_seconds > self._job_timeout_seconds:
                    if job.status in [JobStatus.PENDING, JobStatus.RUNNING]:
                        job.update_status(
                            JobStatus.EXPIRED, error="Job expired due to timeout"
                        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {"total_jobs": len(self._jobs), "by_status": {}, "by_endpoint": {}}
