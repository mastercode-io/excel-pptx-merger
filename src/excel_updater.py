...
class ExcelUpdater:
    ...
    def update_excel(self, update_data: Dict[str, Any], config: Dict[str, Any]) -> str:
        """
    ...
    def _update_subtable(self, sheet: Worksheet, subtable_config: Dict[str, Any], update_data: Dict[str, Any]) -> None:
        ...
        if "data_update" not in subtable_config:
            self._log_warning(f"Skipping subtable '{subtable_name}' - no data_update configuration")
            return
        ...
        if subtable_type == "key_value_pairs":
            self._update_key_value_pairs_with_offsets(sheet, base_row, base_col, update_config, data)
        elif subtable_type == "table":
            self._update_table_with_offsets(sheet, base_row, base_col, update_config, data)

    def _find_update_location(self, sheet: Worksheet, header_search_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _find_by_cell_address(self, sheet: Worksheet, config: Dict[str, Any]) -> Dict[str, Any]:
        """Direct cell address method."""
        cell_address = config["cell"]
        try:
            row, col = self._parse_cell_address(cell_address)
            self._log_info(f"Found location by cell address {cell_address}: row={row}, col={col}")
            return {"row": row, "col": col, "found": True, "address": cell_address}
        except Exception as e:
            self._log_error(f"Invalid cell address {cell_address}: {e}")
            return {"found": False}

    def _find_by_contains_text(self, sheet: Worksheet, config: Dict[str, Any]) -> Dict[str, Any]:
        """Text search method."""
        search_text = config["text"]
        search_column = config["column"]
        search_range = config.get("search_range", f"{search_column}1:{search_column}100")
        
        try:
            for cell in sheet[search_range]:
                for cell_obj in cell:
                    if cell_obj.value and search_text.lower() in str(cell_obj.value).lower():
                        self._log_info(f"Found location by text '{search_text}' at {cell_obj.coordinate}")
                        return {
                            "row": cell_obj.row,
                            "col": cell_obj.column,
                            "found": True,
    ...
    def _update_key_value_pairs_with_offsets(self, sheet: Worksheet, base_row: int, base_col: int, 
                                           config: Dict[str, Any], data: Dict[str, Any]) -> None:
    ...
    def _update_table_with_offsets(self, sheet: Worksheet, base_row: int, base_col: int,
                                 config: Dict[str, Any], data: List[Dict[str, Any]]) -> None:
            ...
            for row_idx, row_data in enumerate(data):
                ...
                for mapping_key, field_config in column_mappings.items():
                    ...
                    if field_name in row_data:
                        ...
                        self._update_cell(sheet, cell_address, row_data[field_name], field_type)

    def _update_cell(self, sheet: Worksheet, cell_address: str, value: Any, field_type: str) -> bool:
        ...
        try:
            ...
            if field_type == "text":
                ...
                try:
                    ...
                    cell.value = str(value) if value is not None else ""
                    self._log_warning(f"Could not convert '{value}' to number, saved as text in {cell_address}")
                ...
                # Default to text
                cell.value = str(value) if value is not None else ""
                self._log_success(f"Updated {cell_address} with default text: '{value}'")
    ...
    def _insert_image(self, sheet: Worksheet, cell_address: str, image_data: Any) -> bool:
        ...
        try:
            ...
            if isinstance(image_data, str):
                if image_data.startswith('file://') or os.path.exists(image_data):
                    ...
                    try:
                        ...
                        self._log_error(f"Failed to process local file {image_data}: {e}")
                        sheet[cell_address].value = "!#VALUE"
                        return False
                    ...
                    self._log_info(f"Inserted text instead of image at {cell_address}: {image_data}")
                    return True
            else:
                # Unknown format - use text
                sheet[cell_address].value = str(image_data)
                self._log_warning(f"Unknown image format, inserted as text at {cell_address}")
                return True
            ...
            self._log_success(f"Inserted image at {cell_address} (size: {final_size[0]}x{final_size[1]})")
            return True
    ...
    def _load_local_image(self, file_path: str) -> bytes:
        ...
        try:
            ...
            if file_path.startswith('file://'):
                import urllib.parse
                file_path = urllib.parse.unquote(file_path[7:])  # Remove 'file://' prefix
    ...
    def _process_image_with_sizing(self, pil_img):
        ...
        try:
            ...
            if original_width <= max_width_pixels and original_height <= max_height_pixels:
                # Image is small enough, use original size (copy to new image to avoid fp issues)
                self._log_info(f"Using original image size: {original_width}x{original_height}px")
                ...
                new_img = pil_img.copy()
                new_img.thumbnail((max_width_pixels, max_height_pixels), PILImage.Resampling.LANCZOS)
                new_size = new_img.size
                self._log_info(f"Resized image from {original_width}x{original_height}px to {new_size[0]}x{new_size[1]}px")
    ...
    def _adjust_row_height(self, sheet, row, image_height_pixels):
        ...
        try:
            ...
            if required_height_points > current_height:
                ...
                self._log_info(f"Adjusted row {row} height from {current_height:.1f} to {required_height_points:.1f} points")
            else:
                self._log_info(f"Row {row} height sufficient: {current_height:.1f} points (needed: {required_height_points:.1f})")
            ...
            for subtable in sheet_config.get("subtables", []):
                ...
                for field in required_base_fields:
                    if field not in subtable:
                        raise ValidationError(f"Subtable missing required field: {field}")
                
                # Must have either data_update or data_extraction
                if "data_update" not in subtable and "data_extraction" not in subtable:
                    raise ValidationError(f"Subtable '{subtable['name']}' must have either 'data_update' or 'data_extraction'")
                
                # If using data_extraction, warn and skip
                if "data_extraction" in subtable and "data_update" not in subtable:
                    logger.warning(f"Subtable '{subtable['name']}' has data_extraction but not data_update - skipping")
                ...
                if method == "cell_address":
                    if "cell" not in header_search:
                        raise ValidationError(f"cell_address method requires 'cell' field in {subtable['name']}")
                elif method == "contains_text":
                    required = ["text", "column", "search_range"]
                    for field in required:
                        if field not in header_search:
                            raise ValidationError(f"contains_text method requires '{field}' field in {subtable['name']}")
                else:
                    raise ValidationError(f"Unsupported method '{method}' in {subtable['name']}")
                ...
                if "data_update" in subtable:
                    ...
                    if "column_mappings" not in data_update:
                        raise ValidationError(f"column_mappings required in data_update for {subtable['name']}")
    ...
    def _add_update_log_sheet(self) -> None:
        ...
        try:
            ...
            # Add headers
            headers = ["Timestamp", "Operation", "Cell/Range", "Status", "Details", "Original Value", "New Value"]
            ...
            for row_idx, log_entry in enumerate(self.update_log, 2):
                ...
                log_sheet.cell(row=row_idx, column=6, value=log_entry.get("original_value", ""))
                log_sheet.cell(row=row_idx, column=7, value=log_entry.get("new_value", ""))
            
            self._log_info(f"Created update_log sheet with {len(self.update_log)} entries")
    ...
    def _save_updated_workbook(self) -> str:
        ...
        try:
            ...
            output_path = os.path.join(output_dir, f"updated_{base_name}_{timestamp}.xlsx")
    ...
    def _log_success(self, message: str, cell: str = "", original_value: Any = "", new_value: Any = "") -> None:
        """Log success message."""
        entry = {
            "timestamp": datetime.now().isoformat(),
            "operation": "UPDATE",
    ...
    def _log_existing_content(self) -> None:
        ...
        try:
            ...
            for sheet_name in self.workbook.sheetnames:
                ...
                self._log_info(f"Sheet '{sheet_name}': {sheet_images} images, {sheet_charts} charts, {sheet_data_cells} data cells")
            
            self._log_info(f"Total content: {total_images} images, {total_charts} charts, {total_data_cells} data cells")
    ...
    def _verify_sheet_preservation(self, sheet_name: str) -> None:
        ...
        try:
            ...
            images = len(sheet._images) if hasattr(sheet, '_images') and sheet._images else 0
            charts = len(sheet._charts) if hasattr(sheet, '_charts') and sheet._charts else 0
            ...
            self._log_info(f"Verified preservation of sheet '{sheet_name}': {images} images, {charts} charts, {data_cells} data cells")
            ...
            for row in sheet.iter_rows():
                for cell in row:
                    if str(cell.value) == "!#VALUE":
                        value_errors += 1
                        self._log_warning(f"Found !#VALUE error in {sheet_name}:{cell.coordinate}")
            
            if value_errors > 0:
                self._log_error(f"Sheet '{sheet_name}' has {value_errors} !#VALUE errors - possible data loss")
                
        except Exception as e:
            self._log_warning(f"Failed to verify sheet preservation for '{sheet_name}': {e}")
    ...
    def _insert_image_v2(self, sheet: Worksheet, cell_address: str, image_data: Any) -> bool:
        ...
        try:
            ...
            if isinstance(image_data, str):
                if image_data.startswith('file://') or os.path.exists(image_data):
                    ...
                    try:
                        ...
                        if os.path.exists(file_path):
                            ...
                            final_image_path = file_path
                            self._log_info(f"Using original file directly: {file_path} (size: {original_size[0]}x{original_size[1]})")
                        else:
                            raise ValueError(f"File not found: {file_path}")
                    except Exception as e:
                        self._log_error(f"Failed to process local file {image_data}: {e}")
                ...
                elif image_data.startswith('data:image') or image_data.startswith('http'):
                    ...
                    try:
                        ...
                        self.temp_image_files.append(temp_image_path)
                        final_image_path = temp_image_path
                        self._log_info(f"Saved to temp file: {temp_image_path} (size: {original_size[0]}x{original_size[1]})")
                    except Exception as e:
