...
def extract_handler(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            "🔄 Starting extract job processing - calling original endpoint with internal_data"
        )
        ...
        if isinstance(result, tuple):
            ...
            if status_code == 200:
                ...
                raise APIError(
                    f"Extract endpoint returned status {status_code}: {response_data}"
                )
            ...
            try:
                ...
                if isinstance(result, tuple):
                    ...
                    if status_code == 200:
                        ...
                        raise APIError(
                            f"Merge endpoint returned status {status_code}: {response_data}"
                        )
            ...
            try:
                ...
                if isinstance(result, tuple):
                    ...
                    if status_code == 200:
                        ...
                        raise APIError(
                            f"Update endpoint returned status {status_code}: {response_data}"
                        )
...
class JobHandlerRegistry:
    ...
    def register_handler(
        self, endpoint: str, handler: Callable[[Dict[str, Any]], Dict[str, Any]]
    ):
